...
export class Montante {
    constructor(
        ...
        if (this.matriz.tipo === 1 && !vectorOriginal) throw new Error('La matriz debe tener un vector de soluciones original para poder calcular la solucion');
    }
    ...
    * resolver() {
            ...
            if (pivActual === 0) {
                ...
                for (let i = k + 1; i < this.matriz.m; i++) {
                    if (matriz[i][k] !== 0) {
                        ...
                        yield [matriz, i, k, `Se intercambian las filas ${i + 1} y ${k + 1} ya que el elemento ${i + 1}, ${k + 1} es diferente de 0`, pivActual, pivAnterior];
                ...
                if (matriz[k][k] === 0) {
                    yield [matriz, -1, -1, `La matriz es singular, por lo que no tiene una solucion única el sistema${this.matriz.tipo === 1 ? ', la matriz no tiene adjunta ni inversa' : ''} y el determinante es 0`
                        , null, null];
                    return;
                }
            }
                ...
                if (i !== k) {
                    for (let j = k; j < matriz.n; j++) {
                        ...
                        if (j === k) {
                            ...
                            matriz[i][j] = Math.round((pivActual * matriz[i][j] - matriz[k][j] * prevMatriz[i][k]) / pivAnterior);
        ...
        yield [matriz, -1, -1,
            `El determinante de la funcion es ${matriz[0][0]} ${(this.divisor === 1) ? '' : (`/${this.divisor} = ${determinante}`)}`,
            null, null];
        ...
        if (this.matriz.tipo === 0) {
            ...
            const vectorSolucion = matriz.map(row => row.slice(this.matriz.m).reduce((a, b, i) => a + b * this.vectorOriginal![i], 0) / determinante);
            let mensajeVector = '';
            ...
            for (let i = 0; i < this.matriz.m; i++) {
                ...
                for (let m = 0; m < this.matriz.m; m++) {
                    if (m === 0) mensajeVector += `${this.vectorOriginal![m]} * ${matriz[i][m + this.matriz.m]}`;
            ...
            yield [matriz, -1, -1, `El vector solucion es ${mensajeVector} = ${vectorSolucion.join(', ')}`, null, null];
        }
    }
}
